...
public class DBUtils {
     ...
     * 删除已经存在的视频记录
     */
    public boolean delVideoPlay(int chaterId,int videoId,String userName){
        boolean delSuccess = false;
        int row = db.delete(SQLiteHelper.U_VIDEO_PLAY_LIST, " chapterId=? AND videoId=? AND userName=?", new String[]{ chaterId + "", videoId + "", userName});
        if (row > 0){
            delSuccess = true;
        }
        return  delSuccess;
